...
There is a detailed [tutorial](https://developers.google.com/analytics/solutions/google-analytics-super-proxy) available to set up Google Analytics superProxy. But, if you are interested to just quickly get your Nirdisto-based blog display page views, follow along. These steps were tested on a Linux machine. If you are running Windows, you can use the Git bash terminal to run Unix-like commands.
...
4.  Enter any random key for `XSRF_KEY`, your `config.py`{: .filepath} should look similar to this
    ...
    AUTH_CONFIG = {
      ...
      'OAUTH_REDIRECT_URI': '%s%s' % (
        'https://nirdisto-test-XXXXXX.ue.r.appspot.com',
        '/admin/auth'
      )
    }
...
5.  From inside the `src/`{: .filepath} directory, deploy the app
    ...
    target project:  [nirdisto-test-XXXX]
    target service:  [default]
    target version:  [VESRION_NUM]
    target url:      [https://nirdisto-test-XXXX.ue.r.appspot.com]
    ...
    Deployed service [default] to [https://nirdisto-test-XXXX.ue.r.appspot.com]
...
## Configure Nirdisto to Display Page View

Once all the hard part is done, it is very easy to enable the Page View on Nirdisto theme. Your superProxy dashboard should look something like below and you can grab the required values.

![superproxy-dashboard](/posts/20210103/05-superproxy-dashboard.png){: width="1210" height="694"}

Update the `_config.yml`{: .filepath} file of [**Nirdisto**][nirdisto-homepage] project with the values from your dashboard, to look similar to the following:
...
[nirdisto-homepage]: https://github.com/md-palash-hossain/jekyll-theme-nirdisto/
